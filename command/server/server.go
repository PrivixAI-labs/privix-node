...
import (
	...
	"github.com/PrivixAI-labs/Privix-node/command/server/config"
	"github.com/PrivixAI-labs/Privix-node/command/server/export"
...
func GetCommand() *cobra.Command {
	serverCmd := &cobra.Command{
		Use:     "server",
		Short:   "The default command that starts the Privix core client, by bootstrapping all modules together",
		PreRunE: runPreRun,
		Run:     runCommand,
	}
...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(
		...
		"the data directory used for storing Privix core client data",
	)
	...
	cmd.Flags().StringVar(
		&params.rawConfig.JSONRPCAuthToken,
		"auth-hex",
		"",
		"the auth token for the JSON-RPC server",
	)

	cmd.Flags().StringSliceVar(
		&params.rawConfig.JSONRPCAllowlist,
		"allowlist",
		[]string{},
		"the JSON-RPC method allowlist (e.g. 'eth_*,web3_*')",
	)
...
func setLegacyFlags(cmd *cobra.Command) {
	// Legacy privix ibft base timeout flag
	cmd.Flags().Uint64Var(
