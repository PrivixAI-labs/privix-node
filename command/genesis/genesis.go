...
import (
	...
	"github.com/PrivixAI-labs/Privix-node/command/genesis/predeploy"
...
func setFlags(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		...
		"the directory for the Privix core genesis data",
	)
	...
	cmd.Flags().Uint64Var(
		...
		"the ID of the chain (only used for privix ibft consensus)",
	)
	...
	// privix ibft Validators
	{
		...
		cmd.Flags().StringArrayVar(
			...
			[]string{},
			"addresses to be used as privix ibft validators, can be used multiple times. "+
	...
	// PoS
	{
		cmd.Flags().BoolVar(
			...
			posFlag,
			false,
			"the flag indicating that the client should use Proof of Stake privix ibft. Defaults to "+
