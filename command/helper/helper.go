...
import (
	...
	txpoolOp "github.com/PrivixAI-labs/Privix-node/txpool/proto"
...
// GetIBFTOperatorClientConnection returns the privix ibft operator client connection
...
func RegisterJSONRPCFlag(cmd *cobra.Command) {
	cmd.PersistentFlags().String(
		command.JSONRPCFlag,
		fmt.Sprintf("%s:%d", LocalHostBinding, server.DefaultJSONRPCPort),
