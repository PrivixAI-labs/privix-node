...
import (
	...
	"github.com/PrivixAI-labs/Privix-node/network/dial"
	"github.com/PrivixAI-labs/Privix-node/network/discovery"
...
const (
	...
	// maxGossipMessageSize is the maximum size of a gossip message in bytes.
	maxGossipMessageSize = 1048576 // 1MB
...
// defaultPeerScoreParams returns the default peer score parameters and thresholds for the pubsub.
// These parameters are based on the libp2p specification and are considered production-ready.
func defaultPeerScoreParams() (*pubsub.PeerScoreParams, *pubsub.PeerScoreThresholds) {
	scoreParams := &pubsub.PeerScoreParams{
		// Topics contains the scoring parameters for each topic.
		Topics: make(map[string]*pubsub.TopicScoreParams),

		// TopicScoreCap is a cap on the contribution of topic scores to the total score.
		TopicScoreCap: 32.72,

		// AppSpecificScore is a function that allows the application to score peers.
		AppSpecificScore:  func(p peer.ID) float64 { return 0 },
		AppSpecificWeight: 1,

		// IPColocationFactorWeight is the weight of the IP colocation factor.
		// This is used to penalize peers that are on the same IP address.
		IPColocationFactorWeight:    -100,
		IPColocationFactorThreshold: 5,

		// BehaviourPenaltyWeight is the weight of the behaviour penalty.
		// This is used to penalize peers that misbehave.
		BehaviourPenaltyWeight:    -10,
		BehaviourPenaltyDecay:     0.999,
		BehaviourPenaltyThreshold: 5,

		// DecayInterval is the interval at which the score decays.
		DecayInterval: 1 * time.Second,
		DecayToZero:   0.01,

		// RetainScore is the time to retain a peer's score after it disconnects.
		RetainScore: 12 * time.Hour,
	}

	scoreThresholds := &pubsub.PeerScoreThresholds{
		// GossipThreshold is the score below which a peer's gossip is ignored.
		GossipThreshold: -10,
		// PublishThreshold is the score below which a peer is not allowed to publish messages.
		PublishThreshold: -50,
		// GraylistThreshold is the score below which a peer is graylisted and its messages are ignored.
		GraylistThreshold: -100,
		// AcceptPXThreshold is the score above which peer exchange messages are accepted.
		AcceptPXThreshold: 10,
		// OpportunisticGraftThreshold is the score above which a peer is opportunistically grafted into the mesh.
...
func NewServer(logger hclog.Logger, config *Config) (*Server, error) {
	...
	maxPeers := int(config.MaxInboundPeers + config.MaxOutboundPeers)
	// set low watermark to 80% of maxPeers
	lowPeers := int(float64(maxPeers) * 0.8)
	...
	scoreParams, scoreThresholds := defaultPeerScoreParams()
