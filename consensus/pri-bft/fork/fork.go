...
const (
	// Keys in privix ibft Configuration
	KeyType          = "type"
	KeyTypes         = "types"
...
var (
	ErrUndefinedIBFTConfig = errors.New("privix ibft config is not defined")
	errInvalidBlockTime    = errors.New("invalid block time provided")
)

// privix ibft Fork represents setting in params.engine.ibft of genesis.json
...
// GetIBFTForks returns privix ibft fork configurations from chain config
func GetIBFTForks(ibftConfig map[string]interface{}) (IBFTForks, error) {
	// no fork, only specifying privix ibft type in chain config
