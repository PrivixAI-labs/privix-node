...
import (
	...
	"github.com/PrivixAI-labs/Privix-node/consensus/pri-bft/proto"
	...
	"github.com/PrivixAI-labs/Privix-node/syncer"
...
var (
	ErrInvalidHookParam           = errors.New("invalid privix ibft hook param passed in")
...
// backendIBFT represents the privix ibft consensus mechanism object
type backendIBFT struct {
	...
	operator       *operator              // Reference to the gRPC service of privix ibft
	transport      transport              // Reference to the transport protocol

	// Dynamic References
	forkManager       forkManagerInterface  // Manager to hold privix ibft Forks
...
// Start starts the privix ibft consensus
...
// Close closes the privix ibft consensus mechanism, and does write back to disk
...
// SetHeaderHash updates hash calculation function for privix ibft
...
func (i *backendIBFT) logFork(
	...
	if lastSigner != nil && signer != nil && lastSigner.Type() != signer.Type() {
		i.logger.Info("privix ibft validation type switched", "old", lastSigner.Type(), "new", signer.Type())
	}
}
...
// verifyProposerSeal verifies ProposerSeal in privix ibft Extra of header
