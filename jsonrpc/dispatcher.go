...
var (
	errMethodNotFound = NewMethodNotFoundError("method not found")
)
...
func (d *Dispatcher) isAllowed(method string) bool {
	if len(d.params.allowlist) == 0 {
		return true
	}

	for _, allowed := range d.params.allowlist {
		if strings.HasSuffix(allowed, "*") {
			if strings.HasPrefix(method, strings.TrimSuffix(allowed, "*")) {
				return true
			}
